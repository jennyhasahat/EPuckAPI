		...
		void* data)
{
			...
			this->addr))
	{
		...
		if( !(strncmp(req->prop, "color", (size_t)req->prop_count) ))
				...
				if( mod )
					{
						...
						uint64_t data = mod->GetWorld()->SimTimeNow();
						memcpy(reply.value, &data, reply.value_count);
			...
			{
				PRINT_WARN1("Property \"%s\" is not accessible. Options are \"color\" or \"time\" for getting the simulation time.", req->prop);
				return(-1);
			}
